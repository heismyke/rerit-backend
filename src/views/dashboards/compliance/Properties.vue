  ...
  <div class="min-h-screen flex bg-[#f5f7fa]">
    ...
    <div class="flex-1 flex flex-col">
      <header class="h-14 bg-white border-b border-[#e5e7eb] flex items-center justify-between px-6 shrink-0">
        <div class="flex items-center gap-4"><span class="text-[#6b7280] text-sm">{{ selectedRole?.name }}</span><span class="text-[#d1d5db]">/</span><span class="text-[#1f2937] text-sm font-medium">Properties</span></div>
      ...
      <main class="flex-1 p-6">
        <div class="bg-white border border-[#e5e7eb] rounded-lg">
          <div class="px-6 py-4 border-b border-[#e5e7eb] flex items-center justify-between"><h2 class="text-[13px] font-semibold text-[#1f2937]">Property Listings</h2><button @click="showAddModal = true" class="btn-primary text-[11px]">Add Property</button></div>
            ...
            <table class="w-full">
              ...
              <tbody class="divide-y divide-[#f3f4f6]">
                <tr v-for="property in paginatedProperties" :key="property.id" class="hover:bg-[#f9fafb]">
                  ...
                  <td class="table-cell"><div class="flex gap-2"><button @click="openViewModal(property)" class="px-3 py-1 text-[11px] bg-[#f3f4f6] text-[#374151] rounded hover:bg-[#e5e7eb]">View</button><button @click="startAudit(property)" class="px-3 py-1 text-[11px] bg-[#2D5A27] text-white rounded hover:bg-[#6a0707]">Audit</button></div></td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    <Teleport to="body">
      <div v-if="showAddModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
          <div class="bg-[#2D5A27] px-6 py-4 flex justify-between items-center"><h3 class="text-base font-semibold text-white">Add Property</h3><button @click="showAddModal = false" class="text-white/80 hover:text-white">✕</button></div>
          <div class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div><label class="block text-[11px] font-medium text-gray-600 mb-1.5">Type</label><select v-model="newProperty.type" class="input-field w-full"><option>Residential</option><option>Commercial</option><option>Mixed Use</option></select></div>
          ...
          <div class="px-6 py-4 border-t border-gray-100 flex gap-3 justify-end"><button @click="showAddModal = false" class="px-4 py-2 text-[11px] border border-gray-300 rounded-lg hover:bg-gray-50">Cancel</button><button @click="handleAddProperty" class="px-4 py-2 text-[11px] bg-[#2D5A27] text-white rounded-lg hover:bg-[#1e3d1a]">Add</button></div>
        </div>
      </div>
    </Teleport>
        ...
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
          ...
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
              ...
              <div><p class="text-[11px] text-gray-500">Status</p><span class="px-2 py-0.5 text-[11px] font-medium rounded-full" :class="{'bg-green-50 text-green-700': selectedProperty?.status === 'Verified', 'bg-yellow-50 text-yellow-700': selectedProperty?.status === 'Pending'}">{{ selectedProperty?.status }}</span></div>
