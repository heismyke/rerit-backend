...
const auditCases = ref([
  ...
  { id: 'AUD-2024-004', property: 'Plot 8, Banana Island', owner: 'Folake Adeyemi', auditor: 'Michael Brown', priority: 'Critical', status: 'Flagged', started: '2024-01-08', due: '2024-01-22' },
])
...
const handleAddCase = () => {
  ...
  auditCases.value.unshift({ id: newId, auditor: 'Current User', started: new Date().toISOString().split('T')[0], ...newCase.value })
...
const handleUpdateCase = () => {
  ...
  if (index !== -1) { auditCases.value[index] = { ...auditCases.value[index], ...editCase.value }; showToast('Audit case updated') }
  ...
  <div class="min-h-screen bg-[#f5f6fa] flex">
            ...
            <table class="w-full">
              ...
              <tbody class="divide-y divide-[#f3f4f6]">
                <tr v-for="audit in paginatedCases" :key="audit.id" class="hover:bg-[#f9fafb]">
                  ...
                  <td class="table-cell"><div class="flex gap-2"><button @click="openViewModal(audit)" class="px-3 py-1 text-[11px] bg-[#f3f4f6] text-[#374151] rounded hover:bg-[#e5e7eb]">View</button><button @click="openEditModal(audit)" class="px-3 py-1 text-[11px] bg-green-50 text-green-700 rounded hover:bg-green-100">Edit</button></div></td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    <Teleport to="body">
      <div v-if="showAddModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
          <div class="bg-[#B90B0B] px-6 py-4 flex justify-between items-center"><h3 class="text-base font-semibold text-white">Create Audit Case</h3><button @click="showAddModal = false" class="text-white/80 hover:text-white">✕</button></div>
    ...
    <Teleport to="body">
      <div v-if="showEditModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
          <div class="bg-[#B90B0B] px-6 py-4 flex justify-between items-center"><h3 class="text-base font-semibold text-white">Edit Audit Case</h3><button @click="showEditModal = false" class="text-white/80 hover:text-white">✕</button></div>
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
    ...
    <Teleport to="body">
      <div v-if="showViewModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md">
          <div class="bg-[#B90B0B] px-6 py-4 flex justify-between items-center"><h3 class="text-base font-semibold text-white">Audit Case Details</h3><button @click="showViewModal = false" class="text-white/80 hover:text-white">✕</button></div>
          <div class="p-6 space-y-4">
            <div class="grid grid-cols-2 gap-4">
